...
def draw_TSNE(features, y_true, label_names, output_fname, title=None, tsne_plot_count=1200):
    ...
    tsne_result = TSNE(n_components=2, verbose=1, perplexity=40, n_iter=400, learning_rate=300).fit_transform(feature)
    ...
    for c in classes:
        plt.scatter(data['x'][data['label'] == c], data['y'][data['label'] == c],
                    c=COLOR_PALETTES[int(c)], marker='*', s=40)
...
def draw_ROC(logits, y_true, label_names, output_fname, title=None):
    classes = np.unique(y_true)
    ...
    if n_classes != 2:
        ...
        plt.plot([0, 1], [0, 1.05], 'k--', lw=lw)

    else:
        lw = 2
        plt.plot(fpr[0], tpr[0], color='darkorange',
                 lw=lw, label='ROC curve (area = %0.2f)' % roc_auc[0])
        plt.plot([0, 1], [0, 1.05], color='navy', lw=lw, linestyle='--')
