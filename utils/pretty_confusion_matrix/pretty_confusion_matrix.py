...
def configcell_text_and_colors(
    ...
    if (col == (ccl - 1)) or (lin == (ccl - 1)):
        ...
        # the text color of the last line and the last column
        dic["color"] = "g"
        lis_kwa.append(dic)
        dic = text_kwargs.copy()
        dic["color"] = "r"
...
def pp_matrix(
    ...
    if output_fname:
        ...
        img = np.fromstring(canvas.tostring_rgb(), dtype=np.uint8).reshape(int(height), int(width), 3)
        return img
