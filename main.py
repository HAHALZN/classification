...
from objects.evaluate import evaluate
from objects.dataloader import get_train_test_loaders
...
def main_worker(args):
    ...
    # data loading
    train_loader, test_loader = get_train_test_loaders(args)
    ...
    if args.arch.startswith('resnet'):
        ...
        model.classifier[1] = nn.Conv2d(in_channels, args.num_classes, kernel_size=(1, 1), stride=(1, 1))
    elif args.arch.startswith('densenet'):
        in_features = model.classifier.in_features
        model.classifier = nn.Linear(in_features, args.num_classes)
    elif args.arch.startswith('googlenet'):
        model.fc = nn.Linear(1024, args.num_classes)
    ...
    if args.evaluate:
        evaluate(test_loader, model, criterion, epoch, None, args)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.save and ((epoch + 1) % args.evaluate_freq == 0 or (epoch + 1) == args.epochs):
            acc1 = evaluate(test_loader, model, criterion, epoch, writer, args)
